...
export default class Layout extends Component {
  ...
  query = conditions => {
    ...
    axios.get(API.queryFormLayouts, { params: conditions }).then(res => {
  ...
  handleEdit = async item => {
    ...
    const url = item.id ? API.updateFormLayout : API.saveFormLayout
  ...
  handleDelete = item => {
    this.setState({ loading: true })
    axios.post(`${API.deleteFormLayout}/${item.id}`).then(res => {
      if (res === '200') {

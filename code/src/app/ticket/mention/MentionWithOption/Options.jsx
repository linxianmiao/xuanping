...
export default class Options extends Component {
  ...
  handleSave = async () => {
    ...
    if (!inputValue || inputValue.length === 0 || inputValue.trim().length === 0) {
      message.error(i18n('ticket.forms.pinput', '请输入') + i18n('ticket.detail.opinion', '意见'))
      return
    }

    const res = await axios.post(`${API.saveCustomOpinion}`, { opinion: inputValue })
  ...
  handleDelete = async (e, text) => {
    ...
    const res = await axios.post(`${API.deleteCustomOpinion}`, { opinion: text })

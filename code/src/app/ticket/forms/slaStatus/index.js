...
import { getSlaOverdueName, msToTime } from '~/logic/olaAndSla'

class SlaStatus extends React.Component {
  render() {
    const { fieldMinCol, field, formLayoutType, olaAndSlaInfo, source } = this.props
    const { sla: slaInfo } = olaAndSlaInfo || {}
    ...
    return (
      ...
      >
        {source === 'formset' ? (
          ...
          <div className="detail-looks-sheet-content">
            {/* <i
              // className="overdue-icon overdue-icon-sla"
              className="overdue-icon overdue-icon-ola"
              style={{ background: slaInfo?.markColor }}
            /> */}
            <span className="ticket-list-table-th-status">

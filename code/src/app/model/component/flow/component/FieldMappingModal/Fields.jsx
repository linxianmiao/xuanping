  ...
  renderList = () => {
    ...
    return nextData.map(item => {
      ...
      if (otherSelected.length > 0) {
        ...
        if (
          (availabeTypes.includes('treeSel') || availabeTypes.includes('cascader')) &&
          item.id !== otherSelected[0].id
        ) {
          disabled = true
        }
      }
      ...
      return (
        <Menu.Item key={item.code} disabled={disabled} title={item.name}>
          {item.name}
        </Menu.Item>
      )
    })
    ...
    return (
      <div className="u4-transfer-list" style={listStyle}>
        <div className="u4-transfer-list-header">
          {title}
          <span className="u4-transfer-list-header-selected">{List.length || 0}</span>
        </div>

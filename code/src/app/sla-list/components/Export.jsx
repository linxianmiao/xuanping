...
const Export = ({
  ...
  const handleDownload = exportId => {
    let url = source === 'sla' ? API.getSlaExportExcel : API.getOlaExportExcel
    ...
    Modal.success({
      ...
      content: (
        <div>
          ...
          <a onClick={() => download(url)}>{`${source}.xlsx`}</a>
        </div>
      )
    })
  ...
  const getExportProgress = async exportId => {
    const url = source === 'sla' ? API.getSlaExportProgress : API.getOlaExportProcress
  ...
  const handleExport = async () => {
    ...
    const url = source === 'sla' ? API.exportSLA : API.exportOla
    const params = source === 'sla' ? window.SLA_STATISTICS_FILTERS : window.OLA_LIST_FILTERS

...
class FieldGroupEditableSelect extends Component {
  static defaultProps = {
    clearOnClose: false,
    style: undefined,
    value: undefined,
  ...
  query = conditions => {
    ...
    axios.get(API.queryDictionaryData('field_layout'), { params: conditions }).then(res => {
  ...
  handleEdit = async item => {
    ...
    const url = item.id ? API.update_field_layouts : API.save_field_layouts
  ...
  handleDelete = item => {
    this.setState({ loading: true })
    axios.post(`${API.delete_field_layouts}/${item.id}`).then(res => {
      if (res === '200') {
  ...
  render() {
    ...
    return (
      <Select
        ...
        canDelete={item => !item.fieldTotal || item.fieldTotal === 0}
...
export default FieldGroupEditableSelect

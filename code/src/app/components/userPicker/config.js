...
// 获取人员得接口
export async function getUsersNew(params, store, extendQuery, method) {
  const { userInfo: kw, departId, groupId, pageIndex: pageNo, pageSize, userType } = params
  const res = await store.getNewList(
    _.assign({}, { kw, departId, groupId, pageNo, pageSize, type: 1, userType }, extendQuery),
    method
  )
  const records = _.map(res.list, (item) => {
    const { id: userId, account, name: realname, mail: email, userDepartment: departs } = item

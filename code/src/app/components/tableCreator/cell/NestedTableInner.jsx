...
import ImportButton from '../content/ContentForForm/ImportButton'
import FormView from '../content/ContentForForm/FormView'
import TableView from '../content/ContentForForm/TableView'
import OutsideSelect from './outsieSelect'
import Dictionary from './Dictionary'
import ReadOnly from './ReadOnly'
import NormalCellWrapper from '../content/ContentForForm/NormalCellWrapper'
import uuid from '~/utils/uuid'
import { parseHideRules, isMatchHideRule, validator } from '../content/logic'
import TableStore from '../content/ContentForForm/TableStore'
...
class ContentForForm extends Component {
  ...
  constructor(props) {
    ...
    const { ticketId, caseId, modelId, fieldCode, pageFlag, copyTicketId } = props
    // 创建环节都不分页
    ...
    this.state = {
      fields: JSON.parse(sessionStorage.getItem('cachedFields')) || [],
      allData: {},
      tableQueryFields: JSON.parse(sessionStorage.getItem('cachedFields')) || []
    }
  }
  ...
  handleChange = (val, column, record, selected = [], from = 'nestedTable') => {
    ...
    console.log('hhhhhhhhhhhhhhhh', nextData, column, val, record)
    ...
    console.log('nestedTable store sata', nextData, column, val, record, from)
      ...
      return (
        <OutsideSelect
          ...
          onChange={(val, selected) =>
            this.handleChange(val, column, record, selected, this.props.from)
          }
        />
      )
    }
    ...
    if (column.type === 'listSel' && field.tabStatus === '2') {
      return (
        <Dictionary
          ...
          size="small"
          value={record[column.value]}
          onChange={(val) => this.handleChange(val, column, record, [], this.props.from)}
        />
      )
    }

...
export default class UserPickerExtendButton extends Component {
  handleCurrent = (tabs) => {
    ...
    // 防止多次点击
    switch (tab) {
      case 0 : falt = _.some(groups, group => group.groupId === 'currentGroup'); break
      case 1 : falt = _.some(users, user => user.userId === 'currentUser'); break
      case 2 : falt = _.some(departs, depart => depart.id === 'currentDepart'); break
      default : falt = false
    }
  ...
  render() {
    ...
    return (
      ...
      <Tooltip title={getTootipTitle(tabs)}>
        <Button
          onClick={() => { this.handleCurrent(tabs) }}
          disabled={disabled}
          size={size}>
